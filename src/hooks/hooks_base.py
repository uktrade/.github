...
class Hook(ABC):
    ...
    def _skip_check(self) -> bool:
        """If this check is ran in the .github repository, it will always fail as we use a local hook implementation
    ...
    def validate_hook_settings(self) -> bool:
        if self._skip_check():
            logger.debug("This hook is being run inside the .github repo, the validate hooks settings can be ignored")
            return True
        ...
        with open(PRE_COMMIT_FILE, "r", encoding="utf-8") as file:
            ...
            dbt_hook_repo = list(filter(lambda x: "https://github.com/uktrade/.github" in x["repo"], config["repos"]))
            if not dbt_hook_repo:
                logger.debug("File %s does not contain the dbt hooks repo", PRE_COMMIT_FILE)
                return False
            if len(dbt_hook_repo) != 1:
                logger.debug("File %s can only contain one .github repo entry", PRE_COMMIT_FILE)
                return False
